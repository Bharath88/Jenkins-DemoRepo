...
public class WF_APRV_WRK_LSTImpl extends  WF_APRV_WRK_LSTBaseImpl
...
//{{COMP_CLASS_CTOR
public WF_APRV_WRK_LSTImpl (){
	super();
}

public WF_APRV_WRK_LSTImpl(Session session, boolean makeDefaults)
{
...
//{{COMPONENT_RULES
	public static WF_APRV_WRK_LSTImpl getNewObject(Session session, boolean makeDefaults)
	{
		return new WF_APRV_WRK_LSTImpl(session, makeDefaults);
	}	
...
public void afterInsert(DataObject foObj)
{
	//Write Event Code below this line
   /*
    * If a new row is inserted into the WorkList table, a corresponding row needs to be inserted
    * in the child WorkList Details table.
    */
   WF_APRV_WRK_LST_DETImpl.insertWorklistDetails(getSession(), (WF_APRV_WRK_LSTImpl) foObj);
}
...
//END_EVENT_ADD_LISTENERS}}
    ...
    * @param fsSelfAprvRsct - Value of Self Approval Restriction
    * @param fsSbmtId       - Submitter Id
    * @param fsCreaId       - Creator Id
   ...
   public static boolean insert(Session foSession, char fcType,
         ...
         String fsDocDept, String fsDocType, String fsDocCat , String fsSelfAprvRsct,
         String fsSbmtId,String fsCreaId,long flAprvRuleId)
   {
      return insert(foSession, fcType, fsAssignee, fboolRole, fiAprvLevel,
            fsDisplayPage, fbReason, flCommentId, foAsgnDate, fsFwdUserId,
            foFwdDate, fsDocCode, fsDocId, fiDocVersionNo, fsDocDept,
            fsDocType, fsDocCat, fsSelfAprvRsct, fsSbmtId, fsCreaId,
            flAprvRuleId, null, null, null, 3, null);
   }
    ...
    * @param fsCreaId       - Creator Id
    * @param flAprvRuleId   - the value for APRV_RULE_ID
    * @param foWarningDate  - Date from which Warning can be issued
    * @param foEsclOneDate  - Date after which Escalation Level 1 can be performed
    * @param foEsclTwoDate  - Date after which Escalation Level 2 can be performed
    * @param fiPriority       Priority value of current record.
    * @param fsPriorityReason Reason for assigning the Priority.
    * @return boolean       - status on the success of the insert
    */
   public static boolean insert(Session foSession, char fcType,
         String fsAssignee, boolean fboolRole, int fiAprvLevel, String fsDisplayPage,
         ...
         VSDate foFwdDate, String fsDocCode, String fsDocId, int fiDocVersionNo,
         String fsDocDept, String fsDocType, String fsDocCat , String fsSelfAprvRsct,
         String fsSbmtId,String fsCreaId,long flAprvRuleId, VSDate foWarningDate,
         VSDate foEsclOneDate, VSDate foEsclTwoDate, int fiPriority, String fsPriorityReason)
   {
      ...
      try
      {
         ...
         loWorklistRec.setWARN_ELIG_DT(foWarningDate);
         loWorklistRec.setESCL_1_ELIG_DT(foEsclOneDate);
         loWorklistRec.setESCL_2_ELIG_DT(foEsclTwoDate);
		 //Insert into Priority only if the passed in value is a valid value, otherwise
         //retain the defaulted value.
         if( ( fiPriority!=0 ) )
         {
            loWorklistRec.setPRIORITY(fiPriority);
         }
         loWorklistRec.setPRIORITY_REASON(fsPriorityReason);
   ...
   //@SuppressAbstract
      {
         ...
         else
         {
            ...
            switch (miDI_ACTN_CD)
            {
               ...
               case DOC_ACTN_OVERRIDE :
               case DOC_ACTN_REM_OVERRIDE :
               ...
               case WL_CHANGE_PRIORITY:
                  String lsPriorityReas = getSession().getProperty(WL_PRIORITY_REASON_VAL);
                  if(AMSStringUtil.strIsEmpty(lsPriorityReas))
                  {
                     return true;
                  }
                  String[] laryPriority = getSession().getProperty(WL_PRIORITY_VAL).split("~");
                  int liOldPriority = Integer.parseInt(laryPriority[0]);
                  int liNewPriority = Integer.parseInt(laryPriority[1]);

                  getSession().removeProperty(WL_PRIORITY_REASON_VAL);
                  getSession().removeProperty(WL_PRIORITY_VAL);

                  //Set edited Priority fields on Worklist Record
                  this.setPRIORITY(liNewPriority);
                  this.setPRIORITY_REASON(lsPriorityReas);
				  this.save();
                  //Log Priority Change as Document Comment
                  addDocComment(liOldPriority, liNewPriority, lsPriorityReas);
         ...
         String fsDocCd, String fsDocDept, String fsDocId, String fsDocVers)
   {
      ...
      try
      {
         if( fiAction == DOC_ACTN_APPROVE || fiAction == DOC_ACTN_REJECT
               || fiAction == DOC_ACTN_REJECT_ALL )
         {
            //Security data validation for workflow approvals.
            if( isWorkflowAprvDataAuth( loSession.getUserID(), fiAction ) )
            {
               lboolAuthorized = AMSSecurity.actionAuthorized(
         ...
         {
            lboolAuthorized = AMSSecurity
                  .actionAuthorized( loSession.getUserID(), fsDocCd, fiAction );
         }
      }
      catch (Exception loEx)
      {
      ...
      try
      {
         loDocHeader = getAMSDocHeader(loSession, fsDocCat,
      ...
      catch (Exception loEx)
      {
         ...
         switch (fiAction)
         {
            case DOC_ACTN_APPROVE:
               ...
               {
                  moAMSLog.debug(
                     "after docApprove(), lboolAsynchronousProcess="
                     + lboolAsynchronousProcess ) ;
               }
               if(lboolAsynchronousProcess)
               {
                  if ( moAMSLog.isDebugEnabled() )
                  {
                     moAMSLog.debug(
                        "loDocHeader.getData( ATTR_DOC_PHASE_CD ).getint()="
                        + loDocHeader.getData( ATTR_DOC_PHASE_CD ).getint()
                        + ",loDocHeader.getData( ATTR_DOC_STA_CD ).getint()="
                        + loDocHeader.getData( ATTR_DOC_STA_CD ).getint() ) ;
                  }
                  loDocHeader.save() ;
                  if ( moAMSLog.isDebugEnabled() )
            ...
            case DOC_ACTN_REJECT:
               ...
               break;
            case DOC_ACTN_REJECT_ALL:
               if (lboolAction = loIntWf.docRejectAll(loDocHeader, false))
               {
                  lboolAction = loDocHeader.processAutoSubmit(DOC_PHASE_DRAFT,
                        DOC_STA_CD_REJECTED);
               }
               break;
            default:
   ...
   public void performEscalatedRouting(int fiEscalationLevel, WF_APRV_SHImpl foWorksheet)
   {
      ...
      boolean lboolIsEscalateAssigneeRole  = foWorksheet.getESCL_ASGN_ROLE_FL(fiEscalationLevel, liApprovalLevel);

      //Perform Escalated Routing
      escalateItem(fiEscalationLevel, lsEscalateAssignee, lboolIsEscalateAssigneeRole );
   }// end of method
    ...
    *
    * @param fiEscalationLevel Escalation level
    * @param fsAssignee User Id or Role Id to whom escalation has to be performed
    * @param fboolIsEscalateAssigneeRole If Escalate Assignee is Approval Role then pass in true, else pass in false when it is User.
    */
   private void escalateItem(int fiEscalationLevel, String fsAssignee, boolean fboolIsEscalateAssigneeRole)
   {
      ...
      loExcludeVector.add("LOCK_USID");

      // Perform Escalated Routing
      WF_APRV_WRK_LSTImpl loEscalatedRouteItem = WF_APRV_WRK_LSTImpl
      ...
      loEscalatedRouteItem.setASSIGNEE_FL(fboolIsEscalateAssigneeRole);
   ...
   /**
    * Sets a Unique ID for the WRK_LST_ID field using AMSUniqNum whenever a new row is inserted.
    *
    * Initially this code resided in the Formula Derivation of the field.
    * It has been moved here because any field having formula cannot be used in a
    * Relationship.
    *
    * WRK_LST_ID is the foreign key for the child DataObject WF_APRV_WRK_LST_DET.
    */
   protected void setUniqueNumForWrkLst()
   {
      try
      {
         setWRK_LST_ID( AMSUniqNum.getUniqNum("WF_WL_A") );
      }
   ...
   /**
    * Adds/Updates a Document Comment to the selected Worklist Document using the value of field
    * Priority Reason. For each Approval Level a new Document Comment entry is added with the
    * Subject "Lvl X Prty Chg" with X denoting the Approval Level.
    * Note that for each Approval Level only one comment will be maintained.
    * Hence, if an existing entry exists for the same Approval Level it will be updated with the
    * value of Priority Reason
    *
    * @param fiOldPriority The old Priority value of Worklist item.
    * @param fiNewPriority The new Priority value that is being assigned to the Worklist item.
    * @param fsPriorityReason The value of Priority Reason field which will be added as a
    * Comment Message on the Document Comments page.
    */
   private void addDocComment(int fiOldPriority, int fiNewPriority, String fsPriorityReason)
   {
      DOC_CMNTImpl loDocCmnt = DOC_CMNTImpl.getNewObject(getSession(), true);
      loDocCmnt.setDOC_CD(getDOC_CD());
      loDocCmnt.setDOC_DEPT_CD(getDOC_DEPT_CD());
      loDocCmnt.setDOC_ID(getDOC_ID());
      loDocCmnt.setDOC_VERS_NO(getDOC_VERS_NO());

      // Get Document Header of the documnet by passing values of the primary keys ( Document Code,
      // Document Dept Code, Document Id and Document Version Number.
      DOC_HDRImpl loDoc = (DOC_HDRImpl)DOC_HDRImpl.getDocHdrByKey( getSession(), getDOC_CD(), getDOC_DEPT_CD(),
            getDOC_ID(), getDOC_VERS_NO());

      loDocCmnt.setDOC_PHASE_CD(loDoc.getDOC_PHASE_CD());

      /*
       * Set the Subject to "LVL X Prty Chng" to signify that this Comment is coming due to the
       * Change Priority action for Level X.
       */
      loDocCmnt.setCMNT_SUB("LVL " + getAPRV_LVL() + " " + WL_DOC_CMNT_SUB);

      /*
       * In the Message field of Comment, the Priority Reason value is set.
       * Along with it additional information about old and new Priority values is also added.
       */
      StringBuilder lsbReas = new StringBuilder(100);
      lsbReas.append("Changed Priority from ")
             .append(CVL_WF_PRIORITYImpl.getPriorityLabel(getSession(), fiOldPriority))
             .append(" to ")
             .append(CVL_WF_PRIORITYImpl.getPriorityLabel(getSession(), fiNewPriority))
             .append(" for Approval Level ")
             .append(getAPRV_LVL());

      loDocCmnt.setCMNT_MSG(lsbReas.toString());
      loDocCmnt.save();
   }
   
   /**
    * This method will verify the workflow approval authorization .
    * If data authorization successful it returns true otherwise returns false.
    *
    * @param fsUserID User ID.
    * @param fiAction Action.
    */
   
   protected boolean isWorkflowAprvDataAuth(String fsUserID,int fiAction )
   {
      Object[] loSecCodeArray = null;
      boolean lboolDataAuthorized = false;
      int liSecReturnCode = 0;
      try
      {
         loSecCodeArray = AMSSecurity.dataAuthorized( fsUserID,
               getData( ATTR_DOC_CD ).getString(), fiAction, getRow(),
               getSecurityOrgPrefixes(), getFiscYearAttrName() );
         liSecReturnCode = ( (Integer)loSecCodeArray[0] ).intValue();
         if( AMS_DEBUG )
         {
            System.err.println( "isWorkflowAprvDataAuth: "
                  + "security return code = " + liSecReturnCode );
         }
         String lsErrorMsg = null;
         switch( liSecReturnCode )
         {
            case 0:
               lboolDataAuthorized = true;
               break;
            case 1:
               // log security violation
               lsErrorMsg = "Action not authorized";
               raiseException( lsErrorMsg );
               lboolDataAuthorized = false;
               break;
            case 2:
               // log security violation
               lsErrorMsg = (String)loSecCodeArray[1];
               raiseException( lsErrorMsg );
               lboolDataAuthorized = false;
               break;
            case 3:
               StringBuffer lsbErrMsg = new StringBuffer( 256 );
               Vector lvRetVector = (Vector)loSecCodeArray[1];
               Enumeration leRetElems = lvRetVector.elements();
               String lsSecRole;
               String lsField;
               boolean lboolFirst = true;
               lsbErrMsg.append( "%c:Q0063,v:" );
               while( leRetElems.hasMoreElements() )
               {
                  lsSecRole = (String)leRetElems.nextElement();
                  lsField = (String)leRetElems.nextElement();
                  if( !lboolFirst )
                  {
                     lsbErrMsg.append( ", " );
                  } /* end if ( !lboolFirst ) */
                  lsbErrMsg.append( lsField );
                  lsbErrMsg.append( " [" );
                  lsbErrMsg.append( lsSecRole );
                  lsbErrMsg.append( "]" );
                  lboolFirst = false;
               } /* end while ( leRetElems.hasMoreElements() ) */
               lsbErrMsg.append( "%" );
               raiseException( lsbErrMsg.toString() );
               lboolDataAuthorized = false;
               break;
            default:
               lboolDataAuthorized = false;
         ...
         if( AMS_DEBUG )
         {
            System.err.println( "Component security in " + getComponentName()
                  + ( ( lboolDataAuthorized ) ? " PASSED"
                        : " FAILED:" + liSecReturnCode ) );
         }
      } catch( Exception loEx)
      {
         lboolDataAuthorized = false;
      }
      return lboolDataAuthorized;
   }/* end isWorkflowAprvDataAuth() */
