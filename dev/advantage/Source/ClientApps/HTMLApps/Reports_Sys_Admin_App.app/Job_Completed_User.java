...
//{{FORM_CLASS_DECL
public class Job_Completed_User extends Job_Completed_UserBase
...
implements AMSBatchConstants
{
   ...
   private static Log moAMSLog = AMSLogger.getLog( Job_Completed_User.class,
...
//{{FORM_CLASS_CTOR
public Job_Completed_User ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
	super(parentApp);
//END_FORM_CLASS_CTOR}}
   }
//{{EVENT_CODE
   ...
   void T2Jobs_Pending_User_beforePageNavigation( PageNavigation nav, VSOutParam cancel, VSOutParam newPage )
   {
      try
      {
         ...
         lsbWhereClause.append(" AND (ITM_TYP=")
            .append(AMSBatchConstants.SYSTEM_BATCH)
            .append(") AND (RUN_STA<>")
...
//{{EVENT_Job_Completed_User_beforeActionPerformed
   void Job_Completed_User_beforeActionPerformed( ActionElement ae, PageEvent evt, PLSRequest preq )
   {
      try
      {
         /*
         ...
         * The value is marked to be true in the beforePageNav adapter method where it is
         ...
         */
         ...
         else if (ae.getName() != null && ae.getName().equals("btnT3Add_New_Job"))
         {
            ...
            if( !AMSPLSUtil. isJobActnAuthorizedForUser( loCurrSession,
               ...
               llCtlgID, AMSCommonConstants.JOB_ACTN_SCHEDULE, true, this ) )
            {
...
//END_EVENT_Job_Completed_User_beforeActionPerformed}}
...
//{{EVENT_Job_Completed_User_sendingResponse
   void Job_Completed_User_sendingResponse( String originalResponse, VSOutParam cancel, VSOutParam newResponse )
   {
      if ( mboolIsLogFileTrans ==true)
      {
         // transition to jog log file display page
         newResponse.setValue(T5Job_Output_Files.startPage().generate());
         cancel.setValue(true);
         mboolIsLogFileTrans =false;
      }
   }
...
//END_EVENT_Job_Completed_User_sendingResponse}}
...
//{{EVENT_ADAPTER_CODE
	...
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			Job_Completed_User_beforeActionPerformed( ae, evt, preq );
		}
	}
	...
	public void sendingResponse ( VSPage obj, String originalResponse, VSOutParam cancel, VSOutParam newResponse ){
		Object source = obj;
		if (source == this ) {
			Job_Completed_User_sendingResponse( originalResponse, cancel, newResponse );
		}
	}
