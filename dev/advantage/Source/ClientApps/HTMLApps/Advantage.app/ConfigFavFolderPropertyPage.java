...
/*
**  ConfigFavFolderPropertyPage
*/

//{{FORM_CLASS_DECL
public class ConfigFavFolderPropertyPage extends ConfigFavFolderPropertyPageBase
...
//{{FORM_CLASS_CTOR
public ConfigFavFolderPropertyPage ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
	super(parentApp);
//END_FORM_CLASS_CTOR}}
   }
...
//{{EVENT_CODE
//{{EVENT_ConfigFavFolderPropertyPage_beforeActionPerformed
void ConfigFavFolderPropertyPage_beforeActionPerformed( ActionElement ae, PageEvent evt, PLSRequest preq )
{
   syncRecordCurrency( T2IN_FAV, preq ) ;
}
//END_EVENT_ConfigFavFolderPropertyPage_beforeActionPerformed}}
//{{EVENT_T2IN_FAV_beforeQuery
/**
 ...
 */
void T2IN_FAV_beforeQuery( VSQuery foQuery ,VSOutParam foResultSet )
{
...
//END_EVENT_T2IN_FAV_beforeQuery}}
...
//{{EVENT_ADAPTER_CODE
	...
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			ConfigFavFolderPropertyPage_beforeActionPerformed( ae, evt, preq );
		}
	}
	public void beforeQuery( DataSource obj, VSQuery query ,VSOutParam resultset){
		Object source = obj;
		if (source == T2IN_FAV) {
			T2IN_FAV_beforeQuery(query , resultset );
		}
	}
//END_EVENT_ADAPTER_CODE}}
   ...
   {
      VSRow       loCopyRow   = T2IN_FAV.getCurrentRow() ;
      ...
      try
      {
         switch ( fiObjectType )
         {
            case ConfigMaster.FAVORITE_TYPE :
            {
               ...
               VSResultSet loResultSet = T2IN_FAV.getResultSet() ;
               VSRow       loLastRow   = loResultSet.last() ;
               VSRow       loNewRow ;
               VSRow       loCurrRow   = T1IN_FAV.getCurrentRow() ;
               int         liSeqNo     = -1 ;
               ...
               loFav.setApplication( AMSFavorite.getApplication( loCurrRow ) ) ;
               loFav.setParentID( AMSFavorite.getID( loCurrRow ) ) ;

               loNewRow = loResultSet.insert() ;
               AMSFavorite.fillNewRow( loNewRow, loFav ) ;

               T2IN_FAV.updateDataSource() ;
   ...
   {
      VSRow                loParentRow = T1IN_FAV.getCurrentRow() ;
      VSResultSet          loResultSet = T2IN_FAV.getResultSet() ;
      VSRow                loLastRow   = loResultSet.last() ;
      ...
      AMSFavorite.setParentID( loNewRow, AMSFavorite.getID( loParentRow ) ) ;
   ...
   public boolean deleteItem( PLSRequest foPLSReq )
   {
      ...
      loDeleteRow = T2IN_FAV.getCurrentRow() ;
