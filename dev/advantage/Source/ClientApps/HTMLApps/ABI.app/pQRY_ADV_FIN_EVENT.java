...
//{{FORM_CLASS_DECL
public class pQRY_ADV_FIN_EVENT extends pQRY_ADV_FIN_EVENTBase

//END_FORM_CLASS_DECL}}
{
   /** This is the logger object */
   private static Log moAMSLog = AMSLogger.getLog( pQRY_ADV_FIN_EVENT.class,
   ...
   private static String SERVICE_FLOWS_ACTION = "serviceFlows";
	...
	//{{FORM_CLASS_CTOR
	public pQRY_ADV_FIN_EVENT ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
		super(parentApp);
	//END_FORM_CLASS_CTOR}}
	}
	...
	//{{EVENT_CODE
	//{{EVENT_T1QRY_ADV_FIN_EVENT_beforeQuery
void T1QRY_ADV_FIN_EVENT_beforeQuery(VSQuery foQuery ,VSOutParam resultset )
{
   ...
   try
   {
       ...
       * "ABI_REFERENCE" will store the corresponding EVENT_ID field value as reference
       * from Maximo source page.
       * Hence use that value for fetching the required row.
       */
      SearchRequest lsr = new SearchRequest();
      String lsWhere = " ADV_FIN_EVENT.EVENT_ID = " +
...
//END_EVENT_T1QRY_ADV_FIN_EVENT_beforeQuery}}
//{{EVENT_pQRY_ADV_FIN_EVENT_beforeGenerate
void pQRY_ADV_FIN_EVENT_beforeGenerate(HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output)
{
	//Write Event Code below this line

   VSRow loRow = T1QRY_ADV_FIN_EVENT.getCurrentRow();
   /*
   ...
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_ADV_FIN_EVENT_ViewSource")).setEnabled(false);
   }
   else
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_ADV_FIN_EVENT_ViewSource")).setEnabled(true);
   }
   ...
   if(mboolUpdating)
   {
      refreshDataSource(T1QRY_ADV_FIN_EVENT);
      mboolUpdating = false;
   }
}
//END_EVENT_pQRY_ADV_FIN_EVENT_beforeGenerate}}
//{{EVENT_pQRY_ADV_FIN_EVENT_beforeActionPerformed
void pQRY_ADV_FIN_EVENT_beforeActionPerformed( ActionElement foActnElem, PageEvent foEvent, PLSRequest preq )
{
	//Write Event Code below this line
   try
   {
      // If Refresh link is clicked, just update the page with latest data.
      if("T1QRY_ADV_FIN_EVENT_Refresh".equals(foActnElem.getName()))
      {
         refreshDataSource(T1QRY_ADV_FIN_EVENT);
      }
         /*
         ...
         if( RELATED_EVENTS_ACTION.equals( foActnElem.getName() ) )
         {
            ...
            if( loRow != null )
            {
               ...
               loTargetPage.doNavigation( CVL_PARTNERSImpl.ADVFIN,
            ...
            if( loRow != null )
            {
                     ...
                     CVL_DIRECTIONImpl.OUTBOUND ) )
               {
                  ...
                  lsbWhere.append( AMSSQLUtil.getANSIQuotedStr(
                        CVL_PARTNER_IDImpl.ADVFIN, AMSSQLUtil.EQUALS_OPER ) );
               }
               else
               {
       ...
       */
      if("T1QRY_ADV_FIN_EVENT_ViewSource".equals(foActnElem.getName()))
      {
         String lsReference = T1QRY_ADV_FIN_EVENT.getCurrentRow().getData("REFERENCE").getString();

         String lsPage = "";
         if(lsReference.split(",")[0].equals("MAXIMO"))
         {
            lsPage = "pQRY_MAXIMO_EVENT";
         ...
         else if(lsReference.split(",")[0].equals("ADVPERMITTING"))
         {
            lsPage = "pQRY_ADV_PERMITTING_EVENT";
            getParentApp().getSession().getORBSession().setProperty("ABI_REFERENCE", lsReference);
         }
       ...
       */
      if("T1QRY_ADV_FIN_EVENTSaveAll".equals(foActnElem.getName()))
      {
         mboolUpdating = true;
      }
   }
...
//END_EVENT_pQRY_ADV_FIN_EVENT_beforeActionPerformed}}
	...
	T1QRY_ADV_FIN_EVENT.addDBListener(this);
	...
	public void beforeGenerate(VSPage obj, HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output){
		Object source = obj;
		if (source == this ) {
			pQRY_ADV_FIN_EVENT_beforeGenerate(docModel, cancel, output);
		}
	}
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			pQRY_ADV_FIN_EVENT_beforeActionPerformed( ae, evt, preq );
		}
	}
	public void beforeQuery( DataSource obj, VSQuery query ,VSOutParam resultset){
		Object source = obj;
		if (source == T1QRY_ADV_FIN_EVENT) {
			T1QRY_ADV_FIN_EVENT_beforeQuery(query , resultset );
		}
	}
