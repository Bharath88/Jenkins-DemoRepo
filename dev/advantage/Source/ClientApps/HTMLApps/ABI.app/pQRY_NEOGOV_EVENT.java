...
//{{FORM_CLASS_DECL
public class pQRY_NEOGOV_EVENT extends pQRY_NEOGOV_EVENTBase
...
//END_FORM_CLASS_DECL}}
{
	...
	//{{FORM_CLASS_CTOR
	public pQRY_NEOGOV_EVENT ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
		super(parentApp);
	//END_FORM_CLASS_CTOR}}
	}
	...
	//{{EVENT_CODE
	//{{EVENT_T1QRY_NEOGOV_EVENT_beforeQuery
void T1QRY_NEOGOV_EVENT_beforeQuery(VSQuery query ,VSOutParam resultset )
{
   ...
   try
   {
      ...
      StringBuffer lsbWhere = new StringBuffer(128);
         lsbWhere.append( " NEOGOV_EVENT.CLIENT_ID = " ).append( "'" + lsReference[1] + "'" )
               .append( " AND NEOGOV_EVENT.TRANSACT_ID = " ).append( "'" + lsReference[2] + "'"  )
               .append( " AND NEOGOV_EVENT.PACKET_ID = " ).append( "'" + lsReference[3] + "'"  );
      if(query.getSQLWhereClause().trim().equals(""))
...
//END_EVENT_T1QRY_NEOGOV_EVENT_beforeQuery}}
//{{EVENT_pQRY_NEOGOV_EVENT_beforeGenerate
void pQRY_NEOGOV_EVENT_beforeGenerate(HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output)
{
	//Write Event Code below this line
   VSRow loRow = T1QRY_NEOGOV_EVENT.getCurrentRow();
   /*
   ...
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_NEOGOV_EVENT_ViewSource")).setEnabled(false);
   }
   else
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_NEOGOV_EVENT_ViewSource")).setEnabled(true);
   }
   ...
   if(mboolUpdating)
   {
      refreshDataSource(T1QRY_NEOGOV_EVENT);
      mboolUpdating = false;
   }
}
//END_EVENT_pQRY_NEOGOV_EVENT_beforeGenerate}}
//{{EVENT_pQRY_NEOGOV_EVENT_beforeActionPerformed
void pQRY_NEOGOV_EVENT_beforeActionPerformed( ActionElement ae, PageEvent evt, PLSRequest preq )
{
	//Write Event Code below this line
   try
   {
      // If Refresh link is clicked, just update the page with latest data.
      if("T1QRY_NEOGOV_EVENT_Refresh".equals(ae.getName()))
      {
         refreshDataSource(T1QRY_NEOGOV_EVENT);
      }
         /*
          ...
          * page.
          */
         if( RELATED_EVENTS_ACTION.equals( ae.getName() ) )
         {
            VSRow loRow = T1QRY_NEOGOV_EVENT.getCurrentRow();
            if( loRow != null )
            {
               ...
               loTargetPage.doNavigation( CVL_PARTNERSImpl.NEOGOV,
                     lsbWhere.toString(), lsdirection, "", "", lsClientId,
                     lsTransactId, lsPacketId );
               evt.setNewPage( loTargetPage );
               evt.setCancel( true );
            }
         }
         /*
          ...
          */
         if( SERVICE_FLOWS_ACTION.equals( ae.getName() ) )
         {
            VSRow loRow = T1QRY_NEOGOV_EVENT.getCurrentRow();
            if( loRow != null )
            {
               String lsdirection = loRow.getData( "DIRECTION" ).getString();
               String lsMessageId = loRow.getData( "TRANSACT_ID" ).getString();
               StringBuffer lsbWhere = new StringBuffer( 64 );
               if( AMSStringUtil.strEqual( lsdirection,
                     CVL_DIRECTIONImpl.OUTBOUND ) )
               {
                  lsbWhere.append( "IN_EVENT_ID " );
                  lsbWhere.append( AMSSQLUtil.getANSIQuotedStr( lsMessageId,
                        AMSSQLUtil.EQUALS_OPER ) );
                  lsbWhere.append( " AND IN_EVENT_PRTNR_ID" );
                  lsbWhere.append( AMSSQLUtil.getANSIQuotedStr(
                        CVL_PARTNER_IDImpl.NEOGOV, AMSSQLUtil.EQUALS_OPER ) );
               }
               else
               {
       ...
       */
      if("T1QRY_NEOGOV_EVENT_ViewSource".equals(ae.getName()))
      {
         String lsReference = T1QRY_NEOGOV_EVENT.getCurrentRow().getData("REFERENCE").getString();
         ...
         loDynTran.setSourcePage(this);
         evt.setNewPage(loDynTran.getVSPage(getParentApp(), getSessionId()));
         evt.setCancel(true);
      }
       ...
       */
      if("T1QRY_NEOGOV_EVENTSaveAll".equals(ae.getName()))
      {
         mboolUpdating = true;
      }
   }
...
//END_EVENT_pQRY_NEOGOV_EVENT_beforeActionPerformed}}
	...
	T1QRY_NEOGOV_EVENT.addDBListener(this);
	...
	public void beforeGenerate(VSPage obj, HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output){
		Object source = obj;
		if (source == this ) {
			pQRY_NEOGOV_EVENT_beforeGenerate(docModel, cancel, output);
		}
	}
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			pQRY_NEOGOV_EVENT_beforeActionPerformed( ae, evt, preq );
		}
	}
	public void beforeQuery( DataSource obj, VSQuery query ,VSOutParam resultset){
		Object source = obj;
		if (source == T1QRY_NEOGOV_EVENT) {
			T1QRY_NEOGOV_EVENT_beforeQuery(query , resultset );
		}
	}
