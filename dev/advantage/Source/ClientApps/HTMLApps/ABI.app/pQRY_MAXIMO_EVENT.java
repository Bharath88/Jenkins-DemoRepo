...
//{{FORM_CLASS_DECL
public class pQRY_MAXIMO_EVENT extends pQRY_MAXIMO_EVENTBase
...
//END_FORM_CLASS_DECL}}
{
   ...
   private static Log moAMSLog = AMSLogger.getLog( pQRY_MAXIMO_EVENT.class,
   ...
   private static String SERVICE_FLOWS_ACTION = "serviceFlows";
	...
	//{{FORM_CLASS_CTOR
	public pQRY_MAXIMO_EVENT ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
		super(parentApp);
	//END_FORM_CLASS_CTOR}}
	}
	...
	//{{EVENT_CODE
	//{{EVENT_pQRY_MAXIMO_EVENT_beforeGenerate
void pQRY_MAXIMO_EVENT_beforeGenerate(HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output)
{
	//Write Event Code below this line

   VSRow loRow = T1QRY_MAXIMO_EVENT.getCurrentRow();
   /*
   ...
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_MAXIMO_EVENT_ViewSource")).setEnabled(false);
   }
   else
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_MAXIMO_EVENT_ViewSource")).setEnabled(true);
   }
   ...
   if(mboolUpdating)
   {
      refreshDataSource(T1QRY_MAXIMO_EVENT);
      mboolUpdating = false;
   }
}
//END_EVENT_pQRY_MAXIMO_EVENT_beforeGenerate}}
//{{EVENT_T1QRY_MAXIMO_EVENT_beforeQuery
void T1QRY_MAXIMO_EVENT_beforeQuery(VSQuery foQuery ,VSOutParam resultset )
{
	//Write Event Code below this line
   try
   {
      ...
      String lsWhere = " MAXIMO_EVENT.MESSAGEID = " +
...
//END_EVENT_T1QRY_MAXIMO_EVENT_beforeQuery}}
//{{EVENT_pQRY_MAXIMO_EVENT_beforeActionPerformed
void pQRY_MAXIMO_EVENT_beforeActionPerformed( ActionElement foActnElem, PageEvent foEvent, PLSRequest preq )
{
	//Write Event Code below this line
   try
   {
      // If Refresh link is clicked, just update the page with latest data.
      if("T1QRY_MERIDIAN_EVENT_Refresh".equals(foActnElem.getName()))
      {
         refreshDataSource(T1QRY_MAXIMO_EVENT);
      }
         /*
         ...
         if( RELATED_EVENTS_ACTION.equals( foActnElem.getName() ) )
         {
            ...
            if( loRow != null )
            {
               ...
               lsbWhere.append( " AND MESSAGEID " );
               lsbWhere.append( AMSSQLUtil.getANSIQuotedStr( lsMessageId,
                     AMSSQLUtil.EQUALS_OPER ) );
               lsbWhere.append( " OR SOURCE_MESSAGEID = " );
               lsbWhere.append( AMSSQLUtil.getANSIQuotedStr( lsMessageId,
                     AMSSQLUtil.EQUALS_OPER ) );
               ...
               loTargetPage.doNavigation( CVL_PARTNERSImpl.MAXIMO,
                     lsbWhere.toString(), lsdirection, "", lsMessageId, "", "",
                     "" );
            ...
            if( loRow != null )
            {
                     ...
                     CVL_DIRECTIONImpl.OUTBOUND ) )
               {
                  ...
                  lsbWhere.append( AMSSQLUtil.getANSIQuotedStr(
                        CVL_PARTNER_IDImpl.MAXIMO, AMSSQLUtil.EQUALS_OPER ) );
               }
               else
               {
       ...
       */
      if("T1QRY_MAXIMO_EVENT_ViewSource".equals(foActnElem.getName()))
      {
         String lsReference = T1QRY_MAXIMO_EVENT.getCurrentRow().getData("REFERENCE").getString();
       ...
       */
      if("T1QRY_MAXIMO_EVENTSaveAll".equals(foActnElem.getName()))
      {
         mboolUpdating = true;
      }
   }
...
//END_EVENT_pQRY_MAXIMO_EVENT_beforeActionPerformed}}
	...
	T1QRY_MAXIMO_EVENT.addDBListener(this);
	...
	public void beforeGenerate(VSPage obj, HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output){
		Object source = obj;
		if (source == this ) {
			pQRY_MAXIMO_EVENT_beforeGenerate(docModel, cancel, output);
		}
	}
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			pQRY_MAXIMO_EVENT_beforeActionPerformed( ae, evt, preq );
		}
	}
	public void beforeQuery( DataSource obj, VSQuery query ,VSOutParam resultset){
		Object source = obj;
		if (source == T1QRY_MAXIMO_EVENT) {
			T1QRY_MAXIMO_EVENT_beforeQuery(query , resultset );
		}
	}
