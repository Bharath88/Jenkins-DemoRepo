...
//{{FORM_CLASS_DECL
public class pQRY_MERIDIAN_EVENT extends pQRY_MERIDIAN_EVENTBase

//END_FORM_CLASS_DECL}}
{
	
   /** This is the logger object */
   private static Log moAMSLog = AMSLogger.getLog( pQRY_MERIDIAN_EVENT.class,
   ...
   // This is the constructor for the generated form. This also constructs
	...
	//{{FORM_CLASS_CTOR
	public pQRY_MERIDIAN_EVENT ( PLSApp parentApp ) throws VSException, java.beans.PropertyVetoException {
		super(parentApp);
	//END_FORM_CLASS_CTOR}}
	}
	...
	//{{EVENT_CODE
	//{{EVENT_T1QRY_MERIDIAN_EVENT_beforeQuery
void T1QRY_MERIDIAN_EVENT_beforeQuery(VSQuery foQuery ,VSOutParam resultset )
{
   ...
   try
   {
      ...
      lsbWhere.append(" MERIDIAN_EVENT.CLIENT_ID = ").append("'" + lsReference[1] + "'")
            .append(" AND MERIDIAN_EVENT.TRANSACT_ID = ").append("'" + lsReference[2] + "'")
            .append(" AND MERIDIAN_EVENT.PACKET_ID = ").append("'" + lsReference[3] + "'");
...
//END_EVENT_T1QRY_MERIDIAN_EVENT_beforeQuery}}
//{{EVENT_pQRY_MERIDIAN_EVENT_beforeGenerate
void pQRY_MERIDIAN_EVENT_beforeGenerate(HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output)
{
	//Write Event Code below this line

   VSRow loRow = T1QRY_MERIDIAN_EVENT.getCurrentRow();
   /*
   ...
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_MERIDIAN_EVENT_ViewSource")).setEnabled(false);
   }
   else
   {
      ((AMSHyperlinkActionElement)getElementByName("T1QRY_MERIDIAN_EVENT_ViewSource")).setEnabled(true);
   }
   ...
   if(mboolUpdating)
   {
      refreshDataSource(T1QRY_MERIDIAN_EVENT);
      mboolUpdating = false;
   }
}
//END_EVENT_pQRY_MERIDIAN_EVENT_beforeGenerate}}
//{{EVENT_pQRY_MERIDIAN_EVENT_beforeActionPerformed
void pQRY_MERIDIAN_EVENT_beforeActionPerformed( ActionElement foActnElem, PageEvent foEvent, PLSRequest preq )
{
   //Write Event Code below this line
   try
   {
      ...
      if("T1QRY_MERIDIAN_EVENT_Refresh".equals(foActnElem.getName()))
      {
         ...
         if( RELATED_EVENTS_ACTION.equals( foActnElem.getName() ) )
         {
            ...
            if( loRow != null )
            {
               ...
               loTargetPage.doNavigation( CVL_PARTNERSImpl.MERIDIAN,
            ...
            if( loRow != null )
            {
                     ...
                     CVL_DIRECTIONImpl.OUTBOUND ) )
               {
                  ...
                  lsbWhere.append( AMSSQLUtil.getANSIQuotedStr(
                        CVL_PARTNER_IDImpl.MERIDIAN, AMSSQLUtil.EQUALS_OPER ) );
               }
               else
               {
       ...
       */
      if("T1QRY_MERIDIAN_EVENT_ViewSource".equals(foActnElem.getName()))
      {
         String lsReference = T1QRY_MERIDIAN_EVENT.getCurrentRow().getData("REFERENCE").getString();
       ...
       */
      if("T1QRY_MERIDIAN_EVENTSaveAll".equals(foActnElem.getName()))
      {
         mboolUpdating = true;
      }
   }
...
//END_EVENT_pQRY_MERIDIAN_EVENT_beforeActionPerformed}}
	...
	T1QRY_MERIDIAN_EVENT.addDBListener(this);
	...
	public void beforeGenerate(VSPage obj, HTMLDocumentModel docModel, VSOutParam cancel , VSOutParam output){
		Object source = obj;
		if (source == this ) {
			pQRY_MERIDIAN_EVENT_beforeGenerate(docModel, cancel, output);
		}
	}
	public void beforeActionPerformed ( VSPage obj, ActionElement ae, PageEvent evt, PLSRequest preq){
		Object source = obj;
		if (source == this ) {
			pQRY_MERIDIAN_EVENT_beforeActionPerformed( ae, evt, preq );
		}
	}
	public void beforeQuery( DataSource obj, VSQuery query ,VSOutParam resultset){
		Object source = obj;
		if (source == T1QRY_MERIDIAN_EVENT) {
			T1QRY_MERIDIAN_EVENT_beforeQuery(query , resultset );
		}
	}
